...
exports.Initialize = initialize = function(app, mongoose) {
    ...
    var controllersPath = path.normalize(app.set('root') + '/models');
    ...
    fs.readdir(controllersPath, function(err, files){
        ...
        files.forEach(function(file){
            ...
            var model = require(path.normalize(app.set('root') + '/models/') + name)[name](mongoose);
            app.models[name] = model;

            // Create the controllers
            var controller = baseController.create(model);
            controller.name = name;
            controller.plural = name + 's';

            var controllerPath = path.normalize(app.set('root') + '/controllers/') + name + '.js';

            path.exists(controllerPath, function(exists){
                controller = exists ? require(controllerPath)[name + 'Controller'](controller, app) : controller;
                app.controllers[name] = controller;

                // Register all the routes
                registerRoutes(app, controller);

                var routesPath = path.normalize(app.set('root') + '/routes/') + name + '.js';
                path.exists(controllerPath, function(exists){
