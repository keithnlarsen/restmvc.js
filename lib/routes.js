...
module.exports = function(app, controller) {
    ...
    app.get('/' + controller.plural + '/:id.:format?', function(request, response, next) {
        controller.show(request.params.id, function(err, instance) {
            if (err)
    ...
    app.get('/' + controller.plural + '.:format??start=:start&limit=:limit', function(request, response, next) {
        controller.index(request.params.start, request.params.limit, function(err, results, pager) {
            if (err) {
                next(new Error('Internal Server Error: see logs for details: ' +  err), request, response);
            }
            else {
    ...
    app.post('/' + controller.plural + '/:id.:format?', function(request, response, next) {
        var entity;
        if (request.params.format){
            if (request.params.format.toLowerCase() == 'json'){
                entity = request.body;
    ...
    app.put('/' + controller.plural + '.:format?', function(request, response, next) {
        var entity;
        if (request.params.format){
            if (request.params.format.toLowerCase() == 'json'){
                entity = request.body;
    ...
    app.del('/' + controller.plural + '/:id', function(request, response, next) {
        controller.destroy(request.params.id, function(err, instance){
            if (err)
                next(new Error('Internal Server Error: see logs for details: ' +  err), request, response);
            else if (!instance)
                next(restErrors.RestError.NotFound.create(controller.name + ' Id: "' + request.params.id + '" was not found.'), request, response);
            else
                response.redirect(controller.plural);
        });
    });
};
